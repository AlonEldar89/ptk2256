...
interface BuildingBFloorPlanProps {
  onApartmentClick: (apartment: string) => void;
  selectedApartment: string | null;
}

const BuildingBFloorPlan: React.FC<BuildingBFloorPlanProps> = ({
  onApartmentClick,
  selectedApartment,
}) => {
  const apartments = ['B1', 'B2', 'B3', 'B4', 'B5'];
      ...
      <Grid container spacing={2}>
        {apartments.map((apt) => (
          ...
          >
            ...
            >
              <img
                src={`${import.meta.env.BASE_URL}floor-plans/building-b/${apt}.jpg`}
...
export default BuildingBFloorPlan; 